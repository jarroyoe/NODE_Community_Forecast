...
realTimeSeries <- read.csv(paste(sep="",paste(sep="_","../Data/timeSeries_communitySize",selectedCommunitySize,"observationError","0.0","rep",selectedTimeSeriesID),".csv"),header=FALSE,sep='\t')[,1:(chosenTrainingSize+20)]

realTimeSeries <- data.frame(Time = rep(1:(chosenTrainingSize+20),selectedCommunitySize),PopID = rep(1:selectedCommunitySize,each = 50),RealValue = as.vector(t(realTimeSeries)))
