...
timeSeriesSpread %>% mutate(ForecastHorizon = Time-TrainingSize) %>% filter(ForecastHorizon>0,TrainingSize<100) %>%  group_by(ArchitectureType,ForecastHorizon,TrainingSize) %>% summarise(MeanSpread=10^mean(log10(Spread)),SESpread=10^sd(log10(Spread))/sqrt(n())) %>%  ggplot(aes(ForecastHorizon,MeanSpread,color=ArchitectureType,shape=as.factor(TrainingSize))) + geom_point(position=position_dodge(width=0.9),size=2)+scale_y_continuous(trans='log10',
                     breaks=trans_breaks('log10', function(x) 10^x),
                     labels=trans_format('log10', math_format(10^.x)))+labs(x="Forecasting Horizon",y="Size of Confidence Interval",color="Architecture Type") + theme_bw() + theme(text = element_text(size=22))
```
