...
selectedTimeSeries[selectedTimeSeries$ModelType == "ARIMA",8] = selectedARIMA$Lower95
selectedTimeSeries[selectedTimeSeries$ModelType == "ARIMA",9] = selectedARIMA$Higher95

selectedTimeSeries[selectedTimeSeries$ModelType == "LSTM",8] = selectedLSTM$Lower95
selectedTimeSeries[selectedTimeSeries$ModelType == "LSTM",9] = selectedLSTM$Higher95

selectedTimeSeries[selectedTimeSeries$ModelType == "EDM",8] = selectedEDM$Lower95
selectedTimeSeries[selectedTimeSeries$ModelType == "EDM",9] = selectedEDM$Higher95
...
```{r}
timeSeriesSpread <- allPredictions %>% filter(Time<=TrainingSize+20) %>% group_by(CommunitySize,ObservationError,ModelType,ArchitectureType,TrainingSize,Time,PopID) %>% summarise(RealValue = RealValue,MeanPrediction = mean(LogDensity),SEPrediction = sd(LogDensity)/sqrt(n())) %>% mutate(Lower95 = MeanPrediction - 1.96*SEPrediction, Higher95 = MeanPrediction + 1.96*SEPrediction) %>% select(-SEPrediction) %>% arrange(TrainingSize,PopID,Time)

selectedARIMA <- ARIMAPredictionsSD %>% mutate(ModelType = "ARIMA",ArchitectureType = "ARIMA") %>% filter(Time<=TrainingSize+20) %>%  group_by(CommunitySize,ObservationError,ModelType,ArchitectureType,TrainingSize,Time,PopID) %>% select(-CommunitySize,-ObservationError,-TimeSeriesID) %>% arrange(TrainingSize,PopID,Time)

selectedLSTM <- LSTMPredictionsSD %>% mutate(ModelType = "LSTM",ArchitectureType = "LSTM") %>% filter(Time<=TrainingSize+20) %>%  group_by(CommunitySize,ObservationError,ModelType,ArchitectureType,TrainingSize,Time,PopID) %>% select(-CommunitySize,-ObservationError,-TimeSeriesID) %>% arrange(TrainingSize,PopID,Time)

selectedEDM <- EDMPredictionsSD %>% mutate(ModelType = "EDM",ArchitectureType = "EDM") %>% filter(Time<=TrainingSize+20) %>%  group_by(CommunitySize,ObservationError,ModelType,ArchitectureType,TrainingSize,Time,PopID) %>% select(-CommunitySize,-ObservationError,-TimeSeriesID) %>% arrange(TrainingSize,PopID,Time)

timeSeriesSpread[timeSeriesSpread$ModelType == "ARIMA",10] = selectedARIMA$Lower95
timeSeriesSpread[timeSeriesSpread$ModelType == "ARIMA",11] = selectedARIMA$Higher95

timeSeriesSpread[timeSeriesSpread$ModelType == "LSTM",10] = selectedLSTM$Lower95
timeSeriesSpread[timeSeriesSpread$ModelType == "LSTM",11] = selectedLSTM$Higher95

timeSeriesSpread[timeSeriesSpread$ModelType == "EDM",10] = selectedEDM$Lower95
timeSeriesSpread[timeSeriesSpread$ModelType == "EDM",11] = selectedEDM$Higher95

timeSeriesSpread <- timeSeriesSpread %>% mutate(Spread = Higher95-Lower95) %>% mutate(CV = abs(Spread/MeanPrediction))

timeSeriesSpread %>% lm(Spread~ModelType+ArchitectureType+TrainingSize+CommunitySize+ObservationError, data = .) %>% 
  ...
  geom_bar(stat='identity') + xlab("Variable") + ylab("Contribution to variation (%)") + theme_bw() + ylim(0,15) + labs(fill = "Term") + theme(text=element_text(size=20))
```
```{r}
