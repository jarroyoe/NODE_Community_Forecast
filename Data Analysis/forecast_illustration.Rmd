...
```{r}
allPredictions <- testPredictions %>% full_join(ARIMAPredictions %>% mutate(ModelType = "ARIMA",ArchitectureType = "ARIMA",RepNumber = 1)) %>% full_join(LSTMPredictions %>% mutate(ModelType = "LSTM",ArchitectureType = "LSTM",RepNumber = 1)) %>% full_join(EDMPredictions %>% mutate(ModelType = "EDM",ArchitectureType = "EDM",RepNumber = 1))

selectedCommunitySize = 10
selectedObservationError = 0
selectedTimeSeriesID = 2
...
p2 <- ggplot(realTimeSeries,aes(Time,RealValue,fill=(PopID)))+geom_point(aes(colour=(PopID)))+ geom_line(data=selectedTimeSeries %>% filter(ModelType == "Time Aware",ArchitectureType == "NODE",TrainingSize == chosenTrainingSize),aes(Time,MeanPrediction,color=(PopID))) + geom_ribbon(data = selectedTimeSeries %>% filter(ModelType == "Time Aware",ArchitectureType == "NODE",TrainingSize == chosenTrainingSize)%>% mutate(Lower95 = max(-10,Lower95),Higher95 = min(10,Higher95)),aes(ymin = Lower95, ymax = Higher95,group = (PopID)),alpha = 0.2) + ggtitle("Time Aware NODE") + ylab("Population Log Density")+ theme_bw()+theme(text = element_text(size=14))+ ylim(-10,10)

p3 <- ggplot(realTimeSeries,aes(Time,RealValue,fill=(PopID)))+geom_point(aes(colour=(PopID)))+ geom_line(data=selectedTimeSeries %>% filter(ModelType == "Time Agnostic",ArchitectureType == "NODE",TrainingSize == chosenTrainingSize),aes(Time,MeanPrediction,color=(PopID))) + geom_ribbon(data = selectedTimeSeries %>% filter(ModelType == "Time Agnostic",ArchitectureType == "NODE",TrainingSize == chosenTrainingSize)%>% mutate(Lower95 = max(-10,Lower95),Higher95 = min(10,Higher95)),aes(ymin = Lower95, ymax = Higher95,group = (PopID)),alpha = 0.2) + ggtitle("Time Agnostic NODE") + ylab("Population Log Density")+ theme_bw()+theme(text = element_text(size=14))+ ylim(-10,10)
...
p5 <- ggplot(realTimeSeries,aes(Time,RealValue,fill=(PopID)))+geom_point(aes(colour=(PopID)))+ geom_line(data=selectedTimeSeries %>% filter(ModelType == "Time Agnostic",ArchitectureType == "UDE",TrainingSize == chosenTrainingSize),aes(Time,MeanPrediction,color=(PopID))) + geom_ribbon(data = selectedTimeSeries %>% filter(ModelType == "Time Agnostic",ArchitectureType == "UDE",TrainingSize == chosenTrainingSize) %>% mutate(Lower95 = max(-10,Lower95),Higher95 = min(10,Higher95)),aes(ymin = Lower95, ymax = Higher95,group = (PopID)),alpha = 0.2) + ggtitle("Time Agnostic UDE") + theme_bw() + ylab("Population Log Density")+ theme(text = element_text(size=14)) + ylim(-10,10)

ggarrange(p1,p2,p3,p4,p5,common.legend = TRUE,legend='none')
```   
...
timeSeriesSpread %>% mutate(ForecastHorizon = Time-TrainingSize) %>% filter(ForecastHorizon>0) %>%  ggplot(aes(ForecastHorizon,Spread,color=ArchitectureType,shape=TrainingSize)) + geom_point(position=position_dodge(width=0.9),size=2)+scale_y_continuous(trans='log10',
                     breaks=trans_breaks('log10', function(x) 10^x),
                     labels=trans_format('log10', math_format(10^.x)))+labs(x="Forecasting Horizon",y="Size of Confidence Interval",color="Architecture Type") + theme_bw() + theme(text = element_text(size=22))
```
