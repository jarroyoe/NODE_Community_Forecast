...
timeSeriesSpread %>% mutate(ForecastHorizon = Time-TrainingSize) %>% filter(ForecastHorizon>0,TrainingSize<100) %>%  group_by(ArchitectureType,ForecastHorizon) %>% summarise(maxSpread = max(Spread),minSpread = min(Spread)) %>% melt(id=c("ArchitectureType","ForecastHorizon")) %>%  ggplot(aes(ForecastHorizon,value,color=ArchitectureType,fill=variable)) + geom_line(size=2)+scale_y_continuous(trans='log10',breaks=trans_breaks('log10', function(x) 10^x),labels=trans_format('log10', math_format(10^.x)))+labs(x="Forecasting Horizon",y="Size of Confidence Interval",color="Architecture Type") + theme_bw() + theme(text = element_text(size=22))
```
#Interval score
```{r}
