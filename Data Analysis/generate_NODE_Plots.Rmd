...
#Data processing
```{r,warning=FALSE}
allPredictions <- testPredictions  %>%  full_join(ARIMAPredictions %>% mutate(ModelType = "ARIMA",ArchitectureType = "ARIMA",RepNumber = 1)) %>% full_join(LSTMPredictions %>% mutate(ModelType = "LSTM",ArchitectureType = "LSTM",RepNumber = 1)) %>% full_join(EDMPredictions %>% mutate(ModelType = "EDM",ArchitectureType = "EDM",RepNumber = 1))%>% filter(TrainingSize<100)
...
RMSEs_smallerWindow %>% ggplot(aes(as.factor(TrainingSize),(NRMSE),fill=as.factor(ArchitectureType))) + geom_violin() + theme_bw() + xlab("Training Size") + ylab("Normalized RMSE") +    scale_y_continuous(trans='log10',
                     breaks=trans_breaks('log10', function(x) 10^x),
                     labels=trans_format('log10', math_format(10^.x))) + theme(text=element_text(size=22))+labs(fill="Architecture Type")
