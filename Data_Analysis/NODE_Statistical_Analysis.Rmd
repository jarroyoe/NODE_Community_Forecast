...
#Ensure all community sizes for a given time series match
finalCommunitySizes <- finalCommunitySizes %>% select(-NumberAtEnd) %>% left_join(finalCommunitySizes %>% group_by(CommunitySize,ObservationError,TrainingSize,TimeSeriesID) %>% summarize(NumberAtEnd=min(NumberAtEnd))) %>% mutate(Factor = NumberPredictedAtEnd/NumberAtEnd)
```
#ANOVA for RMSE
```{r}
